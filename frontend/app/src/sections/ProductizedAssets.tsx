        ...
        >
          ...
          <div className="flex flex-col lg:flex-row gap-12 lg:gap-20">
            ...
            <div className="lg:w-1/3">
              <h2 className="text-3xl lg:text-4xl font-display font-bold text-[color:var(--text-primary)] mb-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {products.map((p, i) => (
                  <motion.button
                    ...
                    className="group flex items-center gap-2 px-4 py-3 glass-card rounded-lg text-left text-sm text-[color:var(--text-primary)] hover:glow-border-blue transition-all duration-300"
                  >
            ...
            <div className="lg:w-2/3 glass-card rounded-2xl p-6 lg:p-8">
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                {/* Garage Column */}
                <div>
                  <h4 className="text-sm font-display font-bold text-[color:var(--text-primary)] mb-4">Garage</h4>
                  <div className="space-y-2">
                    {garageFeatures.map((f) => (
                      <div key={f} className="flex items-center gap-2 text-sm text-[color:var(--text-secondary)] hover:text-[color:var(--text-primary)] transition-colors cursor-default">
                ...
                {/* Features Column */}
                <div>
                  <h4 className="text-sm font-display font-bold text-[color:var(--text-primary)] mb-4">Features</h4>
                  <div className="space-y-2">
                    {featuresList.map((f) => (
                      <div key={f} className="flex items-center gap-2 text-sm text-[color:var(--text-secondary)] hover:text-[color:var(--text-primary)] transition-colors cursor-default">
                        <ArrowRight className="w-3 h-3 text-core-blue" />
              ...
              <div className="mt-8 flex justify-center">
                <div className="relative w-32 h-32">
                  <div className="absolute inset-0 rounded-xl border border-[color:var(--border-default)] bg-gff-gradient-soft animate-pulse-glow" />
                  <div className="absolute inset-2 rounded-lg border border-[color:var(--border-hover)] flex items-center justify-center">
                    <Settings className="w-10 h-10 text-[color:var(--text-secondary)]" />
                  </div>
                  {/* Orbiting dots */}
                  {[0, 1, 2, 3].map((i) => (
                    <motion.div
